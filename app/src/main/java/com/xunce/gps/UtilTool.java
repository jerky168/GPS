...
public class UtilTool {
     ...
     * @throws Exception
     */
    public static Location callGear(Context context, ArrayList<CellInfo> cellIds) throws Exception {
        String result = "";
        JSONObject data = null;

        if (cellIds == null || cellIds.isEmpty()) {
            UtilTool.alert(context, "cell request param null");
            return null;
        }

        try {
            result = UtilTool.getResponseResult(context, "http://www.google.com/loc/json", cellIds);
     ...
     * @throws Exception
     */
    public static String getResponseResult(Context ctx, String path, ArrayList<CellInfo> cellIds)
            throws UnsupportedEncodingException, MalformedURLException, IOException, ProtocolException, Exception {
        String result = "";

        Log.i(ctx.getApplicationContext().getClass().getSimpleName(), "in param: " + getRequestParams(cellIds));

        InputStream inStream=UtilTool.sendPostRequest(path, getRequestParams(cellIds), "UTF-8");
        if(inStream!=null){
            byte[] data = UtilTool.readInputStream(inStream);
